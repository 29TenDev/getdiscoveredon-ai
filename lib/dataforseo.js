    ...
    (task?.result || []).forEach((result) => {
      (result?.items || []).forEach((item) => {
        ...
        if (item.type === "ai_overview") {
          ...
          if (itemStr.includes(brandName.toLowerCase())) {
            aiOverviewMentioned = true;
          }
          // Collect competitor domains cited in AI Overview
          (item.items || []).forEach((ref) => {
            if (ref.url && !ref.url.includes(domain)) {
              try {
...
async function getBusinessProfile(businessName, location) {
  ...
  // Try all known field names for review count
  const reviewCount =
...
// ── 5. On-Page API — scan website for AI-readiness signals ───────────────────
async function getOnPageSignals(domain) {
  ...
  try {
    ...
    // Get pages data for schema/structured data info
    ...
    return {
      ...
      isMobileFriendly: !!(checks.is_https),
      hasSSL: domain.startsWith("https") || !!(checks.is_https),
...
function calculateScores({ llmData, metrics, serpData, bizProfile, onPage }) {
  // ChatGPT — LLM mentions + citation rate + review authority
  ...
  // Perplexity — citation sources + impressions + web presence + on-page
