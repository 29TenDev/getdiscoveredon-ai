...
// ── 1. LLM Mentions Search — correct endpoint + request structure ─────────────
...
// ── 2. Aggregated Metrics — correct endpoint + request structure ──────────────
...
async function getBusinessProfile(businessName, location) {
  ...
  const result = data?.tasks?.[0]?.result?.[0];
  console.log("RATING DATA:", JSON.stringify(result?.rating, null, 2));
...
function buildReport({ businessName, domain, llmData, metrics, serpData, bizProfile, scores }) {
  ...
  return {
    ...
    scores, aiVolume: llmData.aiVolume, impressions: metrics.impressions,
    citationRate: metrics.citationRate, citedSources: llmData.citedSources.slice(0, 10),
    uniqueCitingDomains: uniqueDomains, findings, recommendations, bizProfile, serpData,
  };
}
